 ...
 * admin 资源命令基类(模板方法模式)。
 *
 * 在 init 钩子中固定完成「读取配置档案 → 校验 baseUrl/apiKey → 构建 AdminApiClient」流程,
...
export abstract class AdminBaseCommand extends BaseCommand {
  ...
  async init(): Promise<void> {
    ...
    if (!profile?.baseUrl?.trim() || !profile?.apiKey?.trim()) {
      throw new Error('当前档案未配置 baseUrl 或 apiKey, 请先执行 `wsc config set`')
    }
    this.client = new AdminApiClient(profile.baseUrl.trim(), profile.apiKey.trim())
  }
}
